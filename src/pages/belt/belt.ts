...
export class BeltPage {
 ...
 notifications:  AfoListObservable<any[]>;
  ...
  constructor(public platform: Platform,private screenOrientation: ScreenOrientation,private localNotifications: LocalNotifications,private afoDatabase:AngularFireOfflineDatabase ,private afAuth:AngularFireAuth,public alertCtrl: AlertController,public actionSheetCtrl: ActionSheetController,public navCtrl: NavController,af: AngularFireDatabase, public navParams: NavParams) {
        ...
        handler: data => {
         if(data.number==""){
          ...
          this.orders.push({
            ...
            text: data.name,number:data.number,action:"اضافة",class:"سيور",orderDate:orderDate,orderTime:orderTime
          })
          );
        }
      }
      ...
      },
      {
        ...
        handler: data => {
          ...
          this.orders.update(item.$key ,{name: data.name,number: data.number}).then(
            this.notifications.push({
            text: data.name,number:data.number,action:"تعديل",class:"سيور",orderDate:orderDate,orderTime:orderTime
          })
          );
        }
      }
...
removeProduct(item){
 var orderDate: String = new Date().toLocaleDateString();
    ...
    this.orders.remove(item.$key).then(
            this.notifications.push({
            text: item.name,number:item.number,action:"حذف",class:"سيور",orderDate:orderDate,orderTime:orderTime
          })
          );;
}
