    ...
    buttons: [
      ...
      },
      {
        ...
        if(data.name==""){
          ...
          this.orders.push({
            name: data.name,number:data.number,class:"japan",orderDate:orderDate,orderTime:orderTime
          }).then(
            this.notifications.push({
            text: data.name,number:data.number,action:"اضافة",class:"ياباني",orderDate:orderDate,orderTime:orderTime
          })
          );
        }
      }
      ...
      },
      {
        ...
        if(data.name==""){
          ...
          this.orders.update(item.$key ,{name: data.name,number: data.number}).then(
            this.notifications.push({
            text: data.name,number:data.number,action:"تعديل",class:"ياباني",orderDate:orderDate,orderTime:orderTime
          })
          );
        }
      }
...
removeProduct(item){
 var orderDate: String = new Date().toLocaleDateString();
    ...
    this.orders.remove(item.$key).then(
            this.notifications.push({
            text: item.name,number:item.number,action:"حذف",class:"ياباني",orderDate:orderDate,orderTime:orderTime
          })
          );
}
