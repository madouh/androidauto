...
@Component({
  selector: 'page-oilseal',
  templateUrl: 'oilseal.html',
})
export class OilsealPage {
    ...
    buttons: [
      ...
      },
      {
        ...
        if(data.name==""){
          ...
          this.orders.push({
            name: data.name,number:data.number,class:"oilseal",orderDate:orderDate,orderTime:orderTime
          }).then(
            this.notifications.push({
            text: data.name,number:data.number,action:"اضافة",class:"اويل سيل",orderDate:orderDate,orderTime:orderTime
          })
          );
        }
      }
      ...
      },
      {
        ...
        if(data.name==""){
          ...
          this.orders.update(item.$key ,{name: data.name,number: data.number}).then(
            this.notifications.push({
            text: data.name,number:data.number,action:"تعديل",class:"أويل سيل",orderDate:orderDate,orderTime:orderTime
          })
          );
        }
      }
...
removeProduct(item){
 var orderDate: String = new Date().toLocaleDateString();
    ...
    this.orders.remove(item.$key).then(
            this.notifications.push({
            text: item.name,number:item.number,action:"حذف",class:"اويل سيل",orderDate:orderDate,orderTime:orderTime
          })
          );
}
